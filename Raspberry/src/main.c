...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    CHECK(construire_chemin_to_objet(1, 2, chemin1, &lenChemin1),
          "objet ou dépôt invalide");

    chemin2 = malloc(MAX_CHEMIN * sizeof(int));
    CHECK(construire_chemin_from_object_to_depot(1, 1, 2, chemin2, &lenChemin2),
          "objet ou dépôt invalide");

    chemin3 = malloc(MAX_CHEMIN * sizeof(int));
    CHECK(construire_chemin_from_depot_to_repos(1, chemin3, &lenChemin3),
          "objet ou dépôt invalide");
    ...
    CHECK_COORD_FOR_INVALID;
    // on reste probablement bloqué dans la zone d'attente derrière un autre
    // robot
    state = WAINTING_ZONE;
...
int construire_chemin_from_object_to_depot(int numeroDepot, int numeroAlle,
                                           int numeroObjet, int *chemin,
                                           int *lenChemin) {
  CHECK_NBRE_ALLEE;
  CHECK_NBRE_OBJ;
  CHECK_NBRE_DEPOT;
...
int construire_chemin_from_depot_to_repos(int numeroDepot, int *chemin,
                                          int *lenChemin) {
  CHECK_NBRE_DEPOT;
...
struct coordones navigation(struct coordones c, int *sorties,
                            int nbreDirections) {
  ...
  while (index < nbreDirections) {
    ...
    } else if (is_intersection(cellule)) { // S'il s'agit d'une intersection, on
                                           // suit le plan
#ifdef _DEBUG_
      printf("intersection ");
#endif
