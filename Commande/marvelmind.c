...
float * getPosition (struct MarvelmindHedge * hedge, bool onlyNew){
    uint8_t i,j;
    double xm,ym;
    static float data[2];
   ...
   if (fusionIMU.updated || (!onlyNew))
    {
       ...
       printf("IMU fusion: Timestamp: %s, X=%.3f  Y= %.3f  angle=%.3f\n",
				times,
				(float) x_m, (float) y_m, (float) qx);

       hedge->fusionIMU.updated= false;
    }
}

float * FusionIMU_data(struct MarvelmindHedge * hedge, bool onlyNew){
	struct FusionIMUValue fusionIMU;
	float x_m, y_m;
	float qx;
